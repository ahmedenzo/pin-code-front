...
import { AhmedEditItemDialogComponent } from '../ahmed-modal-dialog/ahmed-edit-item-dialog.component';
...
@Component({
  ...
  styleUrls: ['./ahmed.component.scss']
})
export class AhmedComponent implements OnInit {
  ...
  fetchData(): void {
    this.ahmedService.getAll().subscribe(
      db => {
  ...
  setupForm(): void {
    ...
    formFields.forEach(field => {
      this.ahmedForm.addControl(field, this.fb.control('', Validators.required));
    });
  }
  ...
  openDialog(isEdit: boolean, item: any): void {
    const dialogRef = this.dialog.open(AhmedEditItemDialogComponent, {
      width: '400px',
  ...
  addNewItem(item: any): void {
    this.ahmedService.create(item).subscribe(
      newItem => {
  ...
  updateItem(item: any): void {
    this.ahmedService.update(item.id, item).subscribe(
      updatedItem => {
  ...
  deleteItem(id: number): void {
    this.ahmedService.delete(id).subscribe(
      () => {
