...
import { ProductEditItemDialogComponent } from '../product-modal-dialog/product-edit-item-dialog.component';
...
export class ProductComponent implements OnInit {
  ...
  setupForm(): void {
    ...
    formFields.forEach(field => {
      this.productForm.addControl(field, this.fb.control('', Validators.required));
    });
  }
  ...
  openDialog(isEdit: boolean, item: any): void {
    const dialogRef = this.dialog.open(ProductEditItemDialogComponent, {
      width: '400px',
  ...
  updateItem(item: any): void {
    this.productService.update(item.id, item).subscribe(
      updatedItem => {
